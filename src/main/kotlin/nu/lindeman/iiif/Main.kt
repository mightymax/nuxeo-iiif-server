...
fun Route.viewer()
{
    ...
    get("/viewer/folder") {
        val manifest = "http://localhost:8080/manifest/from/folder/by/uid/c7065ef6-ad5a-4235-8f05-1d179bc1e147"
        call.respond(FreeMarkerContent("mirador.ftl", mapOf("manifest" to manifest)))
    }

    get("/viewer/folder/{uuid}") {
        val manifest = "http://localhost:8080/manifest/from/folder/by/uid/${call.parameters["uuid"]}"
        call.respond(FreeMarkerContent("mirador.ftl", mapOf("manifest" to manifest)))
    }

    get ("/viewer/folder/path/{...}") {
        val manifest = "http://localhost:8080${call.request.uri.replace("/viewer/folder/path", "/manifest/from/folder/by/path")}"
        println(manifest)
        call.respond(FreeMarkerContent("mirador.ftl", mapOf("manifest" to manifest)))
    }
}